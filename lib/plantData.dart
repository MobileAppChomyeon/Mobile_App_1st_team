...
class PlantService {
  ...
  Future<void> addDefaultPlantsIfNeeded() async {
    try {
      // 'plants' 컬렉션 확인
      final plantCollection = _db.collection('plants');
      final snapshot = await plantCollection.get();

      // 데이터가 이미 존재하면 저장하지 않음
      if (snapshot.docs.isNotEmpty) {
        print('Default plants already exist. Skipping initialization.');
        return;
      }
      ...
      for (var i = 0; i < plants.length; i++) {
        ...
        //   print('Invalid plantId for plant: ${plant['species']}');
        //   continue; // 다음 식물로 넘어감
        // }

        await plantCollection.doc(plantId).set(plant);
      }
      print('Default plants added successfully.');
    } catch (e) {
      print('Error adding default plants: $e');
    }
  }

  Future<void> addEncyclopedia() async {
    ...
    try {
      final encyclopedia = _db
          .collection('Users')
          .doc(user.uid)
          .collection('Plants')
          .doc('encyclopedia')
          .collection('plantsList');
      final snapshot = await encyclopedia.get();

      // 데이터가 이미 존재하면 저장하지 않음
      if (snapshot.docs.isNotEmpty) {
        print('이미 도감 생성해서 건너뜀');
        return;
      }
